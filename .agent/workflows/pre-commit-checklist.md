...
# 2. Check linting (if flake8 is configured)
# flake8 src/
...
**Note**: This repo may not have pre-commit hooks configured. If `.pre-commit-config.yaml` doesn't exist, focus on manual checks.
...
- `.pre-commit-config.yaml` - Hook configuration (if exists)
